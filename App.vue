<template>
  <div id="app">
    ...
    <Footer :state="state" v-if="state.mode != 'embed'"/>
  </div>
</template>
...
export default {
  ...
  data(){
    return {
      state:{
        baseURL: 'https://audioplayer.dweet.net',
        baseDemoURL: 'https://code.dweet.net',
        baseVideoURL: 'https://dweet.net',
        baseDomain: 'audioplayer.dweet.net',
  ...
  methods:{
    ...
    loadTrack(fileName){
      this.state.tracks = [{
        ...
        userID: 0,
        author: '',
        trackName: decodeURI(fileName.split('/')[fileName.split('/').length-1]).split('?')[0],
        playlists: '',
        playing: false,
        date: new Date('H:i:s Y-M-D'),
        private: 0,
        autoPlay: false,
        description: '',
        audioFile: fileName,
      ...
      if(1||vars.length>0){
        switch('embed'){//vars[0]){
          case 'embed':
            ...
            this.$nextTick(()=>this.loadTrack(window.location.href.split(window.location.origin+'/')[1]))
            break
          case 'track':
        ...
        if(e.keyCode == 39){
          if(!document.activeElement.classList.contains('textInput')){
            ...
            if(e.shiftKey){
              ...
              let el
              if((el=this.currentPlayingTracks).length)el[0].mp3.currentTime = Math.min(el[0].mp3.duration, el[0].mp3.duration)
            }
          }
        }
        if(e.keyCode == 32){
